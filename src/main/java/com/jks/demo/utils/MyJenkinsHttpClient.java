...
public class MyJenkinsHttpClient implements JenkinsHttpConnection {
     ...
     */
    public MyJenkinsHttpClient(URI uri, CloseableHttpClient client) {
     ...
     */
    public MyJenkinsHttpClient(URI uri, HttpClientBuilder builder) {
     ...
     * @param uri Location of the jenkins server (ex. http://localhost:8080)
     */
    public MyJenkinsHttpClient(URI uri) {
     ...
     */
    public MyJenkinsHttpClient(URI uri, String username, String password) {
     ...
     * @param password Password or auth token to use when connecting
     */
    public MyJenkinsHttpClient(URI uri, HttpClientBuilder builder, String username, String password) {
    ...
    public String getInfo(String path) throws IOException {
        HttpGet getMethod = new HttpGet(uri+context+path);
        HttpResponse response = client.execute(getMethod, localContext);
        jenkinsVersion = ResponseUtils.getJenkinsVersion(response);
        LOGGER.debug("get({}), version={}, responseCode={}", path, this.jenkinsVersion,
                response.getStatusLine().getStatusCode());
    ...
    public JSONObject getProgressiveHtml(String path,String start) throws IOException {
      JSONObject result = new JSONObject();
      HttpPost postMethod = new HttpPost(uri+context+path);
      ...
      LOGGER.debug("get({}), version={}, responseCode={}", path, this.jenkinsVersion,
              response.getStatusLine().getStatusCode());
      Header[] ts = response.getHeaders("X-Text-Size");
      if(ts.length > 0){
    	  result.put("start", ts[0].getValue());
      }else{
    	  result.put("start", 0);
      }
      try {
          httpResponseValidator.validateResponse(response);
          result.put("info", IOUtils.toString(response.getEntity().getContent()));
          return result;
      } finally {
          EntityUtils.consume(response.getEntity());
...
//    @Override
//    public String get(String path) throws IOException {
////        HttpGet getMethod = new HttpGet("http://10.192.43.11:29630/configureClouds");
